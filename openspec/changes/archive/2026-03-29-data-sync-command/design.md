...
Server details: `deploy-user@example.com -p 22` (chrooted, home = `/`). Target path: `/op-cards.ditshej.ch/database/database.sqlite`.
...
Metanet uses port 22. Keeping host, user, port, path as separate keys is cleaner than parsing a connection string.
