...
- [ ] 1.3 Write failing tests for the warnings: trailing name mismatch (always), and `--deck` only — no leader, two leaders, main deck not 50, quantity above 4; plus a test that a short list without `--deck` emits no composition warnings
- [ ] 1.4 Create `ResolveCardsCommand` with `#[Signature('cards:resolve {file?} {--format=json} {--deck}')]` and implement parsing and validation until 1.1–1.3 pass
...
- [ ] 2.3 Separate the leader by `category === 'Leader'`, keeping the remaining entries in input order; cover the leader-not-first case
- [ ] 2.4 Assert all thirteen fields are present per entry, with null preserved

## 3. Output formats

- [ ] 3.1 Write failing tests for the JSON shape: `leader` (null when absent), `cards`, `totals`, `warnings`
- [ ] 3.2 Implement JSON output, written raw so the document is byte-exact
- [ ] 3.3 Write failing tests for the Markdown shape: leader block, card table, effect and trigger section
...
- [ ] 4.1 Run `php artisan test --compact --filter=ResolveCardsCommand`, then the full suite
- [ ] 4.2 Run `vendor/bin/pint --dirty --format agent`
- [ ] 4.3 Resolve the real 51-line Big Mom list (1 leader + 50 main deck) in both formats, with and without `--deck`, and confirm no warnings either way
- [ ] 4.4 Confirm `php artisan cards:resolve deck.txt > deck-data.json` produces valid JSON, verified with `jq empty`
