...
- **The auth-safety guarantee is behavioural, not structural** — "eliminated by construction" holds only because `cards:import` currently writes exclusively to `Pack`/`Card` via `updateOrCreate`. Nothing (no arch/Pest rule, no DB-level restriction) stops a future change to `cards:import` from touching `users`/`sessions`/`personal_access_tokens` again, which would silently reintroduce the original incident through the same command this fix relies on. Mitigated by a regression test (`tests/Feature/Commands/ImportCardsCommandTest.php`, `it('never writes to auth tables')`) asserting a `User` and its Sanctum token survive an import unchanged; a dedicated architecture rule would be a stronger guarantee but is out of scope here.
