...
function setSyncPhpEnv(?string $value): void
{
    if ($value === null) {
        putenv('SYNC_PHP');
...
it('defaults sync_php to php when SYNC_PHP is unset', function () {
    setSyncPhpEnv(null);

    expect((require config_path('import.php'))['sync_php'])->toBe('php');
});

it('defaults sync_php to php when SYNC_PHP is set but blank', function () {
    setSyncPhpEnv('');

    expect((require config_path('import.php'))['sync_php'])->toBe('php');
});

it('uses SYNC_PHP when it is set to a real path', function () {
    setSyncPhpEnv('/opt/php83/bin/php');
